(function (window, document) {
  ...
  function renderTimelineCalendar(items) {
    const monthKey = ensureCalendarMonth();
    const taskItems = items.filter((item) => item.type === "task");
    const datedTasks = taskItems.filter((item) => item.date);
    const undatedTasks = taskItems.filter((item) => !item.date);
    const monthTasks = datedTasks.filter((item) => item.date.startsWith(monthKey));
    const loggedDays = getLogCountsByDate(items);
    const sections = [
      renderCalendarMonth(monthKey, monthTasks),
      renderLogActivity(loggedDays),
    ];

    if (undatedTasks.length) sections.push(renderCalendarUnscheduled(undatedTasks));
    els.timelineList.replaceChildren(...sections);
    els.timelineSummary.textContent = `${monthTasks.length} task date${monthTasks.length === 1 ? "" : "s"} / ${loggedDays.size} logged day${loggedDays.size === 1 ? "" : "s"}`;
  }

  function ensureCalendarMonth() {
    if (!state.timeline.calendarMonth || !/^\d{4}-\d{2}$/.test(state.timeline.calendarMonth)) {
      state.timeline.calendarMonth = todayMonthKey();
    }
    return state.timeline.calendarMonth;
  }

  function todayMonthKey() {
    return Utils.todayInput().slice(0, 7);
  }

  function shiftCalendarMonth(delta) {
    state.timeline.calendarMonth = addMonthsToKey(ensureCalendarMonth(), delta);
    renderTimeline();
  }
  ...
  function renderCalendarMonth(monthKey, items) {
    const [year, month] = monthKey.split("-").map(Number);
    const firstDate = new Date(year, month - 1, 1);
    const daysInMonth = new Date(year, month, 0).getDate();
    const itemsByDate = groupByDate(items).reduce((map, day) => map.set(day.date, day.items), new Map());
    const section = document.createElement("section");
    section.className = "calendar-month calendar-carousel";

    const header = document.createElement("div");
    header.className = "calendar-carousel-header";
    const heading = document.createElement("h3");
    heading.textContent = firstDate.toLocaleDateString([], { month: "long", year: "numeric" });
    const controls = document.createElement("div");
    controls.className = "calendar-carousel-controls";
    controls.append(
      calendarNavButton("Previous month", "<", () => shiftCalendarMonth(-1)),
      calendarNavButton("Current month", "Today", () => {
        state.timeline.calendarMonth = todayMonthKey();
        renderTimeline();
      }),
      calendarNavButton("Next month", ">", () => shiftCalendarMonth(1))
    );
    ...
    const trailingDays = (7 - ((firstDate.getDay() + daysInMonth) % 7)) % 7;
  ...
  function calendarNavButton(label, text, onClick) {
    const button = document.createElement("button");
    button.className = "secondary-button calendar-nav-button";
    button.type = "button";
  ...
  function renderLogActivity(loggedDays) {
    const dates = buildActivityDates(LOG_ACTIVITY_WEEKS);
    const stats = logActivityStats(loggedDays, dates);
    const section = document.createElement("section");
    section.className = "log-activity";

    const header = document.createElement("div");
    header.className = "log-activity-header";
    const title = document.createElement("div");
    const eyebrow = document.createElement("p");
    eyebrow.className = "eyebrow";
    eyebrow.textContent = "Daily log streak";
    const heading = document.createElement("h3");
    heading.textContent = `${stats.loggedDaysInRange} logged day${stats.loggedDaysInRange === 1 ? "" : "s"} in the last ${LOG_ACTIVITY_WEEKS} weeks`;
    title.append(eyebrow, heading);
    const summary = document.createElement("p");
    summary.className = "log-activity-summary";
    ...
    grid.style.setProperty("--activity-weeks", String(LOG_ACTIVITY_WEEKS));
    grid.append(activityLabel(""));

    for (let week = 0; week < LOG_ACTIVITY_WEEKS; week += 1) {
      const weekStart = dates[week * 7];
      const previousWeekStart = dates[(week - 1) * 7];
      const shouldLabel = week === 0 || weekStart.getMonth() !== previousWeekStart.getMonth();
      grid.append(activityLabel(shouldLabel ? weekStart.toLocaleDateString([], { month: "short" }) : ""));
    }

    ["", "Mon", "", "Wed", "", "Fri", ""].forEach((dayLabel, row) => {
      grid.append(activityLabel(dayLabel, "activity-day-label"));
      for (let week = 0; week < LOG_ACTIVITY_WEEKS; week += 1) {
        const date = dates[(week * 7) + row];
        grid.append(renderActivityCell(date, loggedDays));
      }
    });
    ...
    [0, 1, 2, 3, 4].forEach((level) => {
      const chip = document.createElement("span");
      chip.className = `activity-cell level-${level}`;
  ...
  function activityLabel(text, className = "activity-month-label") {
  ...
  function buildActivityDates(weeks) {
    const today = Utils.stripTime(new Date());
    const currentSunday = Utils.addDays(today, -today.getDay());
    const start = Utils.addDays(currentSunday, -((weeks - 1) * 7));
    return Array.from({ length: weeks * 7 }, (_, index) => Utils.addDays(start, index));
  }

  function renderActivityCell(date, loggedDays) {
    const today = Utils.stripTime(new Date());
    const key = Utils.toDateInput(date);
    const count = loggedDays.get(key) || 0;
    const isFuture = date > today;
    const cell = document.createElement("span");
    cell.className = `activity-cell level-${activityLevel(count)}${isFuture ? " future" : ""}`;
    cell.title = `${Utils.formatLongDate(key)}: ${count} entr${count === 1 ? "y" : "ies"}`;
  ...
  function getLogCountsByDate(items) {
    const counts = new Map();
    items.forEach((item) => {
      if (item.type !== "log" || !Utils.isDateInput(item.date)) return;
      counts.set(item.date, (counts.get(item.date) || 0) + 1);
    });
    return counts;
  }

  function logActivityStats(loggedDays, dates) {
    const todayKey = Utils.todayInput();
    let currentStreak = 0;
    let cursor = Utils.parseDate(todayKey);
    while (cursor && loggedDays.has(Utils.toDateInput(cursor))) {
      currentStreak += 1;
      cursor = Utils.addDays(cursor, -1);
    }

    const sortedDates = Array.from(loggedDays.keys()).sort();
    let longestStreak = 0;
    let running = 0;
    let previous = "";
    sortedDates.forEach((dateKey) => {
      const expected = previous ? Utils.toDateInput(Utils.addDays(Utils.parseDate(previous), 1)) : "";
      running = dateKey === expected ? running + 1 : 1;
      longestStreak = Math.max(longestStreak, running);
      previous = dateKey;
    });

    const dateKeysInRange = new Set(dates.map((date) => Utils.toDateInput(date)));
    const loggedDaysInRange = Array.from(loggedDays.keys()).filter((dateKey) => dateKeysInRange.has(dateKey)).length;
    return { currentStreak, longestStreak, loggedDaysInRange };
  }
  ...
  function updateTimelineFilters() {
    state.timeline = {
      ...
      calendarMonth: state.timeline.calendarMonth,
    };
    renderTimeline();
  }
